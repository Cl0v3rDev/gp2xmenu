...
static mp3_hdr_t *add_mp3_hdr(mp3_hdr_t ** list, off_t st_pos, int mp3_chans, int mp3_freq, int mp3_flen, stream_t *s)
{
	mp3_hdr_t *tmp;
	int in_list = 0;
	// vbr support
	{
		unsigned char buf[200], *p;
		int h_id, h_mode, h_freq;
		int s_pos = stream_tell(s);
		...
		h_freq     = (buf[2] >> 2) & 3;
		if( h_id ) {        // mpeg1
		    if( h_mode != 3 ) p = buf + (32+4);
		    else              p = buf + (17+4);
		} else {      // mpeg2
		    if( h_mode != 3 ) p = buf + (17+4);
		    else              p = buf + (9+4);
		}

		is_vbr = 0;
	    if (!memcmp(p, "VBRI", 4) || !memcmp(p, "Xing", 4))
	    {
			int head_flags;
			int is_xing = !memcmp(p, "Xing", 4);
			is_vbr = is_xing ? 1 : 2;
			...
			if(is_xing)
			{
				head_flags = ExtractI4(p + 4);
				p += 8;

				if( head_flags & 1 ) {
					vbr_frames = ExtractI4(p);
					p += 4;
				}

				if( head_flags & 2 ) {
					vbr_bytes = ExtractI4(p);
					p += 4;
				}

				if( head_flags & 4 ) {
					int i;
			        for(i=0;i<100;i++) xing_toc[i] = p[i];
				}
				else
				{
					xing_toc[99] = 0;
				...
				// VBRI
				vbr_bytes = ExtractI4(p + 10);
				vbr_frames = ExtractI4(p + 14);
			}
			...
			if( vbr_frames > 0)
			{
	            ...
	            int ft_den, ft_num;
	            if(h_freq != 0) ft_den = 1;
	            else ft_den = 49;
				ft_num = 1000 * ft_den * frame_samples / mp3_freq;

	            if (vbr_frames <= 0xfffffffful / ft_num)
	                vbr_time = vbr_frames * ft_num / ft_den;
	            else
	                vbr_time = vbr_frames / ft_den * ft_num;
			}
			else
			{
				is_vbr = 0;
...
int demux_audio_open(demuxer_t * demuxer)
{
	...
	while (n < 30000 && !s->eof) {
		...
		if (hdr[0] == 'R' && hdr[1] == 'I' && hdr[2] == 'F' && hdr[3] == 'F') {
			...
			mp3_found = add_mp3_hdr(&mp3_hdrs, st_pos, mp3_chans, mp3_freq, mp3_flen, s);
			if (mp3_found) {
				frmt = MP3;
				break;
			}
...
//    printf("wav: %X .. %X\n",(int)demuxer->movi_start,(int)demuxer->movi_end);
			...
			if ((w->wFormatTag == 0x01) && (w->nChannels == 2)
...
void demux_audio_seek(demuxer_t * demuxer, float rel_seek_secs, int flags)
{
	...
	if(priv->frmt == MP3 && (is_vbr == 1) && (xing_toc[99]))
	{
		// Xing Format
		float time = (flags & 1) ? 0 : priv->last_pts;
		float percent = time * 100000.0f / vbr_time;
		float fa, fb, fx;
		int toc = (int)percent;

		if(flags & 2)
		    time = time + (vbr_time * rel_seek_secs / 1000.0f);
		else
		    time = time + rel_seek_secs;
		if(time < 0) time = 0;

		percent = time * 100000.0f / vbr_time;
		toc = (int)percent;
		if(toc > 99) toc = 99;
		fa = xing_toc[toc];
		if(toc < 99)
		...
		fx = fa + (fb-fa) * (percent - toc);
		pos = (int)((1.0f/256.0f) * fx * vbr_bytes);
		sh_audio->i_bps = vbr_bytes / (vbr_time / 1000.0f);

		priv->last_pts = time;
		sh_audio->delay = priv->last_pts - (ds_tell_pts(demuxer->audio) - sh_audio->a_in_buffer_len) / (float) sh_audio->i_bps;
	}
	else if (priv->frmt == MP3 && is_vbr && !(flags & 2))
	{
		// VBRI Format or Not TOC include
