...
#define ERROR() { \
	...
	if (vorbis_synthesis_headerin(&ov->vi, &vc, &op) < 0) {
		...
		mp_msg(MSGT_DECAUDIO, MSGL_V, "OggVorbis: Bitstream is %d channel%s, %dHz, %dbit/s %cBR\n", (int) ov->vi.channels, ov->vi.channels > 1 ? "s" : "", (int) ov->vi.rate, (int) ov->vi.bitrate_nominal,
		       (ov->vi.bitrate_lower != ov->vi.bitrate_nominal)
		       || (ov->vi.bitrate_upper != ov->vi.bitrate_nominal) ? 'V' : 'C');
		if (rg_gain || rg_peak)
...
static int decode_audio(sh_audio_t * sh, unsigned char *buf, int minlen, int maxlen)
{
	...
	while (len < minlen) {
		...
		while (samples > 0) {
			...
			samples -= bout;
			vorbis_synthesis_read(&ov->vd, bout);	/* tell libvorbis how many samples we actually consumed */
		}
	}
