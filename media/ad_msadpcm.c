...
static ad_info_t info = {
	...
	{
	 0x2,
	 0x0200736d,		// "ms\x00\x02" (MOV files)
	 0xffffffff}
};
...
static int preinit(sh_audio_t * sh_audio)
{
	...
	sh_audio->ds->ss_div = (sh_audio->wf->nBlockAlign - MS_ADPCM_PREAMBLE_SIZE) * 2;
	sh_audio->audio_in_minsize = sh_audio->ds->ss_mul = sh_audio->wf->nBlockAlign;
	return 1;
}
...
static int init(sh_audio_t * sh_audio)
{
	...
	sh_audio->i_bps = sh_audio->wf->nBlockAlign * (sh_audio->channels * sh_audio->samplerate) / sh_audio->ds->ss_div;
...
static int ms_adpcm_decode_block(unsigned short *output, unsigned char *input, int channels, int block_size)
{
	...
	if (input[stream_ptr] > 6)
		mp_msg(MSGT_DECAUDIO, MSGL_WARN, "MS ADPCM: coefficient (%d) out of range (should be [0..6])\n", input[stream_ptr]);
	...
	while (stream_ptr < block_size) {
		...
		predictor = (((sample1[current_channel] * coeff1[current_channel]) + (sample2[current_channel] * coeff2[current_channel])) / 256) + (snibble * idelta[current_channel]);
		CLAMP_S16(predictor);
		...
		idelta[current_channel] = (ms_adapt_table[nibble] * idelta[current_channel]) / 256;
...
static int decode_audio(sh_audio_t * sh_audio, unsigned char *buf, int minlen, int maxlen)
{
	...
	return 2 * ms_adpcm_decode_block((unsigned short *) buf, sh_audio->a_in_buffer, sh_audio->wf->nChannels, sh_audio->wf->nBlockAlign);
}
