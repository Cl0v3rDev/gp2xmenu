...
static void avi_idx_quicksort(AVIINDEXENTRY * idx, int from, int to)
{
	...
	do {
		while (pivot_ofs < AVI_IDX_OFFSET(&idx[lo]))
			lo--;
		while (pivot_ofs > AVI_IDX_OFFSET(&idx[hi]))
			hi++;
		if (hi <= lo) {
			if (hi != lo) {
...
void read_avi_header(demuxer_t * demuxer, int index_mode)
{
	...
	while (1) {
		...
		/*
		   ...
		   memcpy(fcc, &id, 4);
		   fcc[4] = 0;
		   printf("fourcc id %s (%d)\n", fcc, SDL_GetTicks() - ticks);
		   }
		 */
		if (id == mmioFOURCC('L', 'I', 'S', 'T')) {
			...
			if (id == listtypeAVIMOVIE) {
				...
				list_end = 0;
				//printf("Found movie at 0x%X - 0x%X(%d)\n",(int)demuxer->movi_start,(int)demuxer->movi_end, SDL_GetTicks() - ticks);
			}
			continue;
		}
		...
		case ckidSTREAMFORMAT:{
				...
				if (last_fccType == streamtypeVIDEO) {
					sh_video->bih = calloc((chunksize < sizeof(BITMAPINFOHEADER)) ? sizeof(BITMAPINFOHEADER)
							       : chunksize, 1);
					...
					if (sh_audio->wf->cbSize != 0 && wf_size < sizeof(WAVEFORMATEX) + sh_audio->wf->cbSize) {
						sh_audio->wf = realloc(sh_audio->wf, sizeof(WAVEFORMATEX)
								       + sh_audio->wf->cbSize);
					}
					chunksize = 0;
					if (verbose >= 1)
		...
		case ckidAVINEWINDEX:
			...
			if (index_mode && !priv->isodml) {
				...
				//printf("Reading INDEX block, offset %x,  size %d(%d)\n", priv->idx, priv->idx_size, SDL_GetTicks() - ticks);
			}
			break;
		case mmioFOURCC('R', 'I', 'F', 'F'):{
				...
				if (strncmp(riff_type, "AVIX", sizeof riff_type))
					mp_msg(MSGT_HEADER, MSGL_WARN, "** warning: this is no extended AVI header..\n");
				else {
					/*
		...
		mp_msg(MSGT_HEADER, MSGL_DBG2, "list_end=0x%X  pos=0x%X  chunksize=0x%X  next=0x%X\n", (int) list_end, (int) stream_tell(demuxer->stream), chunksize, (int) chunksize + stream_tell(demuxer->stream));
	...
	if (priv->isodml && (index_mode == -1 || index_mode == 0)) {
		...
		mp_msg(MSGT_HEADER, MSGL_INFO, "AVI: ODML: Building odml index (%d superindexchunks)\n", priv->suidx_size);
		...
		for (cx = &priv->suidx[0], i = 0; i < priv->suidx_size; cx++, i++) {
			...
			for (j = 0; j < cx->nEntriesInUse; j++) {
				...
				if (ret1 != 1 || ret2 != 32 || cx->stdidx[j].nEntriesInUse == 0) {
					...
					priv->isodml = 0;
					priv->idx_size = 0;
					mp_msg(MSGT_HEADER, MSGL_WARN, "AVI: ODML: Broken (incomplete?) file detected. Will use traditional index\n");
					goto freeout;
				}
				...
				stream_read(demuxer->stream, (char *) cx->stdidx[j].aIndex, cx->stdidx[j].nEntriesInUse * sizeof(avistdindex_entry));
	...
	if (index_mode >= 2 || (priv->idx_size == 0 && index_mode == 1)) {
		...
		while (1) {
			...
			// update status line:
			{
				...
				if (pos != lastpos) {
					lastpos = pos;
					mp_msg(MSGT_HEADER, MSGL_STATUS, "Generating Index: %3lu %s     \r", (unsigned long) pos, len ? "%" : "MB");
				}
			}
