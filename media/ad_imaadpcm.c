...
static ad_info_t info = {
	...
	{
	 0x11,
	 0x61,				// This format number was used by Duck Corp.
	 0x62,				// but not officially registered with Microsoft.
	 0x34616d69,		// "ima4" (MOV files)
	 0x1100736d,		// "ms\x00\x11" (MOV files)
	 0xffffffff}
};
...
static int preinit(sh_audio_t * sh_audio)
{
	...
	if ((sh_audio->format == 0x11) || (sh_audio->format == 0x61) || (sh_audio->format == 0x1100736d)) {
		sh_audio->ds->ss_div = (sh_audio->wf->nBlockAlign - (MS_IMA_ADPCM_PREAMBLE_SIZE * sh_audio->wf->nChannels)) * 2;
		sh_audio->ds->ss_mul = sh_audio->wf->nBlockAlign;
	} else if (sh_audio->format == 0x62) {
		sh_audio->audio_out_minsize = sh_audio->wf->nBlockAlign * 6;
		sh_audio->ds->ss_div = (sh_audio->wf->nBlockAlign - DK3_ADPCM_PREAMBLE_SIZE) * 8 / 3;
		sh_audio->ds->ss_mul = sh_audio->wf->nBlockAlign;
	} else {
...
static int init(sh_audio_t * sh_audio)
{
	...
	sh_audio->i_bps = (sh_audio->ds->ss_mul * sh_audio->samplerate) / sh_audio->ds->ss_div;
...
static void decode_nibbles(unsigned short *output, int output_size, int channels, int predictor_l, int index_l, int predictor_r, int index_r)
{
	int step[2];
	int predictor[2];
	int index[2];
...
static int qt_ima_adpcm_decode_block(unsigned short *output, unsigned char *input, int channels)
{
	...
	decode_nibbles(output, QT_IMA_ADPCM_SAMPLES_PER_BLOCK * channels, channels, initial_predictor_l, initial_index_l, initial_predictor_r, initial_index_r);

	return QT_IMA_ADPCM_SAMPLES_PER_BLOCK * channels;
}

static int ms_ima_adpcm_decode_block(unsigned short *output, unsigned char *input, int channels, int block_size)
{
	int predictor_l = 0;
	int predictor_r = 0;
	int index_l = 0;
	...
	if (channels == 1)
		...
		for (i = 0; i < (block_size - MS_IMA_ADPCM_PREAMBLE_SIZE * channels); i++) {
			output[channel_index + 0] = input[MS_IMA_ADPCM_PREAMBLE_SIZE * 2 + i] & 0x0F;
			output[channel_index + 2] = input[MS_IMA_ADPCM_PREAMBLE_SIZE * 2 + i] >> 4;
			channel_index += 4;
			channel_counter++;
	...
	decode_nibbles(output, (block_size - MS_IMA_ADPCM_PREAMBLE_SIZE * channels) * 2, channels, predictor_l, index_l, predictor_r, index_r);

	return (block_size - MS_IMA_ADPCM_PREAMBLE_SIZE * channels) * 2;
}

static int dk4_ima_adpcm_decode_block(unsigned short *output, unsigned char *input, int channels, int block_size)
{
	int i;
	int output_ptr;
	int predictor_l = 0;
	...
	decode_nibbles(&output[channels], (block_size - MS_IMA_ADPCM_PREAMBLE_SIZE * channels) * 2 - channels, channels, predictor_l, index_l, predictor_r, index_r);
...
// stereo flavor
static int dk3_ima_adpcm_decode_block(unsigned short *output, unsigned char *input, int block_size)
{
	int sum_pred;
	int diff_pred;
	int sum_index;
...
{
	if (demux_read_data(sh_audio->ds, sh_audio->a_in_buffer, sh_audio->ds->ss_mul) != sh_audio->ds->ss_mul)
		return -1;

	if ((sh_audio->format == 0x11) || (sh_audio->format == 0x1100736d)) {
		return 2 * ms_ima_adpcm_decode_block((unsigned short *)
						     buf, sh_audio->a_in_buffer, sh_audio->wf->nChannels, sh_audio->ds->ss_mul);
	} else if (sh_audio->format == 0x61) {
		return 2 * dk4_ima_adpcm_decode_block((unsigned short *)
						      buf, sh_audio->a_in_buffer, sh_audio->wf->nChannels, sh_audio->ds->ss_mul);
	} else if (sh_audio->format == 0x62) {
		return 2 * dk3_ima_adpcm_decode_block((unsigned short *)
						      buf, sh_audio->a_in_buffer, sh_audio->wf->nBlockAlign);
	} else {
		return 2 * qt_ima_adpcm_decode_block((unsigned short *)
						     buf, sh_audio->a_in_buffer, sh_audio->wf->nChannels);
	}
}
