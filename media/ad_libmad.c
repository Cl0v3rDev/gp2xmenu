...
static ad_info_t info = {
	...
	"based on Xine's libmad/xine_decoder.c",
	{
	 0x50,
	 0x55,
	 0x33706d2e,		// ".mp3" CBR/VBR MP3 (MOV files)
	 0x5500736d,		// "ms\0\x55" older mp3 fcc (MOV files)
	 0xffffffff}
};
...
static int read_frame(sh_audio_t * sh)
{
	...
	do {
		if(len) {
			memmove(sh->a_in_buffer + sh->a_in_buffer_len, buf, len);
			sh->a_in_buffer_len += len;
		}

		while (sh->a_in_buffer_len) {
			int ret;
	...
	} while ((len = ds_get_packet(sh->ds, &buf)) > 0);
...
/*
	if (this->frame.header.samplerate > 30000)
	{
		sh->samplerate = this->frame.header.samplerate;
		this->output_multiply = 1;
	}
	else if (this->frame.header.samplerate > 15000)
	{
		sh->samplerate = this->frame.header.samplerate * 2;
		this->output_multiply = 2;
	}
	else
	{
		sh->samplerate = this->frame.header.samplerate * 4;
		this->output_multiply = 4;
	}
*/
	return 1;
}
...
static int decode_audio(sh_audio_t * sh, unsigned char *buf, int minlen, int maxlen)
{
	...
	while (len < minlen && len + 4608 <= maxlen) {
...

		{
			...
			//len += 2 * 2 * nsamples;
			//buf += 2 * 2 * nsamples;
			//len += 2 * 2 * nsamples * this->output_multiply;
			//buf += 2 * 2 * nsamples * this->output_multiply;
