		...
		if (!demux->audio->sh) {
			...
			if (sh->wf) {
				...
				if (!priv->audio_block_size) {
					...
					if (sh->audio.dwSampleSize == 1 && sh->audio.dwScale == 1 && (sh->wf->nBlockAlign == 1152 || sh->wf->nBlockAlign == 576)) {
...
static int demux_avi_read_packet(demuxer_t * demux, demux_stream_t * ds, unsigned int id, unsigned int len, int idxpos, int flags)
{
	...
	if (ds == demux->audio) {
		if (priv->pts_corrected == 0) {
			if (priv->pts_has_video) {	// we have video pts now
				float delay = 0;
		...
		if (pts_from_bps) {
			pts = priv->audio_block_no * (float) ((sh_audio_t *) demux->audio->sh)->audio.dwScale / (float) ((sh_audio_t *) demux->audio->sh)->audio.dwRate;
		} else {
			pts = priv->avi_audio_pts;	// +priv->pts_correction;
		}
		priv->avi_audio_pts = 0;

		priv->audio_block_no += priv->audio_block_size ? ((len + priv->audio_block_size - 1)
								  / priv->audio_block_size) : 1;
	} else if (ds == demux->video) {	// video
		if (priv->skip_video_frames > 0) {	// drop frame (seeking)
			--priv->skip_video_frames;
			ds = NULL;
		}

		pts = priv->avi_video_pts = priv->video_pack_no * (float) ((sh_video_t *) demux->video->sh)->video.dwScale / (float) ((sh_video_t *) demux->video->sh)->video.dwRate;
	...
	if (ds) {
		/*{
		   ...
		   memcpy(fcc, &id, 4);
		   fcc[4] = 0;
		   printf("id: %s, type: %s, bytes: %d\n", fcc, (ds == demux->audio) ? "audio" : "video", len);
		   } */
...
int demux_avi_fill_buffer(demuxer_t * demux)
{
	...
	do {
		...
		if (ds && (ds->packs + 1 >= MAX_PACKS || ds->bytes + len >= MAX_PACK_BYTES)) {
#if 1				// enter non interleaved mode...[if need...]
			int nimode = 0;
/*
			char fcc[5];
			memcpy(fcc, &id, 4);
			fcc[4] = 0;
			printf("id: %s, type: %s, packs: %d, bytes: %d\n", fcc, (ds == demux->audio) ? "audio" : "video", ds->packs, ds->bytes);
*/
			if (ds == demux->audio) {
				nimode = (demux->video->packs < 5);
			} else {
				nimode = (demux->audio->packs < 5);
			}
...
//    idx=&priv->idx[idx_pos];
		...
		//if (ds == demux->video)
		//    printf("ni_d_avi_v: %d,%d,%d,%X\n", ds->packs, MAX_PACKS, ds->bytes, MAX_PACK_BYTES);
...
demuxer_t *demux_open_avi(demuxer_t * demuxer)
{
	...
	priv->idx_pos_v = 0;
	priv->idx_offset = 0;
	//priv->idx_size = 0; // only test!!!
	if (priv->idx_size > 1) {
		// decide index format:
#if 1
		if ((AVI_IDX_OFFSET(&((AVIINDEXENTRY *) priv->idx)[0]) < demuxer->movi_start || AVI_IDX_OFFSET(&((AVIINDEXENTRY *) priv->idx)[1]) < demuxer->movi_start) && !priv->isodml)
		...
		if (AVI_IDX_OFFSET(&((AVIINDEXENTRY *) priv->idx)[0]) < demuxer->movi_start)
			...
			priv->idx_offset = 0;
#endif
		mp_msg(MSGT_DEMUX, MSGL_V, "AVI index offset: 0x%X (movi=0x%X idx0=0x%X idx1=0x%X)\n", (int) priv->idx_offset, (int) demuxer->movi_start, (int) ((AVIINDEXENTRY *) priv->idx)[0].dwChunkOffset,
...
//  demuxer->endpos=avi_header.movi_end;
	...
	if (priv->idx_size > 0) {
		...
		if (force_ni) {
			...
			priv->idx_pos_a = priv->idx_pos_v = demuxer->movi_start;
			pts_from_bps = 1;	// force BPS sync!
		}
		demuxer->seekable = 0;
	}
...
int force_backward_seek = 0;
int force_resume_seek = 0;
...
// ------------ STEP 1: find nearest video keyframe chunk ------------
	...
	if ((rel_seek_frames < 0) || force_backward_seek) {
		// seek backward
		if (force_backward_seek) {
			if (video_chunk_pos > 0)
				--video_chunk_pos;
			force_backward_seek = 0;
		}
...
//    d_audio->pts=initial_pts_delay+(float)apos/(float)sh_audio->wf->nAvgBytesPerSec;
		...
		mp_msg(MSGT_SEEK, MSGL_V, "SEEK: idx=%d  (a:%d v:%d)  v.skip=%d  a.skip=%d/%4.3f  \n", (int) priv->idx_pos, audio_chunk_pos, video_chunk_pos, (int) priv->skip_video_frames, skip_audio_bytes, skip_audio_secs);
