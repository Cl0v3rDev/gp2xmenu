...
#if 0				// ghcstop delete ==> no subtitle processing
...
void demux_ogg_add_sub(ogg_stream_t * os, ogg_packet * pack)
{
	...
	mp_msg(MSGT_DEMUX, MSGL_DBG2, "\ndemux_ogg_add_sub %02X %02X %02X '%s'\n", (unsigned char) packet[0], (unsigned char) packet[1], (unsigned char) packet[2], &packet[3]);
...
#endif				// ghcstop delete ==> ogg sub title
...
static void demux_ogg_check_comments(demuxer_t * d, ogg_stream_t * os, int id, vorbis_comment * vc)
{
	...
	while (*cmt) {
		hdr = NULL;
		if (!strncasecmp(*cmt, "TITLE=", 6)) {
			hdr = "Title";
			val = *cmt + 6;
		} else if (!strncasecmp(*cmt, "ARTIST=", 7)) {
			hdr = "Artist";
			val = *cmt + 7;
		} else if (!strncasecmp(*cmt, "ALBUM=", 6)) {
			hdr = "Album";
			val = *cmt + 6;
		} else if (!strncasecmp(*cmt, "DATE=", 5)) {
			hdr = "Year";
			val = *cmt + 5;
		} else if (!strncasecmp(*cmt, "GENRE=", 6)) {
			hdr = "Genre";
			val = *cmt + 6;
		} else if (!strncasecmp(*cmt, "ENCODED_USING=", 14)) {
			hdr = "Software";
			val = *cmt + 14;
...
static int demux_ogg_add_packet(demux_stream_t * ds, ogg_stream_t * os, int id, ogg_packet * pack)
{
	...
	if (!os->flac && ((*pack->packet & PACKET_TYPE_HEADER) && (ds != d->audio || (((sh_audio_t *) ds->sh)->format != 0xFFFE || os->hdr_packets >= NUM_VORBIS_HDR_PACKETS))
	...
	while (1) {
		...
		while (ogg_stream_packetout(oss, &op) == 1) {
			...
			if (index_mode == 2 && (flags || (os->vorbis && op.granulepos >= 0))) {
				...
				ogg_d->syncpoints[ogg_d->num_syncpoint].page_pos = (ogg_page_continued(page)
										    && p == 0) ? last_pos : pos;
				ogg_d->num_syncpoint++;
			}
			p++;
...
#   ifdef HAVE_OGGTHEORA
			...
			if (errorCode)
				...
				n_video++;
				mp_msg(MSGT_DEMUX, MSGL_V, "Ogg : stream %d is theora v%i.%i.%i %i:%i, %.3f FPS," " aspect %i:%i\n", ogg_d->num_sub, (int) inf.version_major, (int) inf.version_minor, (int) inf.version_subminor, inf.width,
				       inf.height, sh_v->fps, inf.aspect_numerator, inf.aspect_denominator);
				if (verbose > 0)
...
#   endif			/* HAVE_FLAC */
			...
			if (get_uint32(pack.packet + 96) == 0x05589f80 && pack.bytes >= 184) {
				...
				sh_v->bih = (BITMAPINFOHEADER *) calloc(1, sizeof(BITMAPINFOHEADER));
				sh_v->bih->biSize = sizeof(BITMAPINFOHEADER);
				sh_v->bih->biCompression = sh_v->format = mmioFOURCC(pack.packet[68], pack.packet[69], pack.packet[70], pack.packet[71]);
			...
			// Check new header
		} else if ((*pack.packet & PACKET_TYPE_BITS) == PACKET_TYPE_HEADER && pack.bytes >= (int) sizeof(stream_header) + 1) {
			...
			if (strncmp(st->streamtype, "video", 5) == 0) {
				...
				sh_v->bih = (BITMAPINFOHEADER *) calloc(1, sizeof(BITMAPINFOHEADER));
				sh_v->bih->biSize = sizeof(BITMAPINFOHEADER);
				sh_v->bih->biCompression = sh_v->format = mmioFOURCC(st->subtype[0], st->subtype[1], st->subtype[2], st->subtype[3]);
		...
		while (1) {
			...
			if (precision && is_gp_valid) {
				...
				if (abs(gp - op.granulepos) > rate && (op.granulepos != old_gp)) {
					...
					pos += (gp - op.granulepos) * (pos - old_pos)
					    / (op.granulepos - old_gp);
					if (pos < 0)
						pos = 0;
					if (pos < (demuxer->movi_end - demuxer->movi_start)) {
						do_seek = 1;
